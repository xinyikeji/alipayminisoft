...
Page({
  ...
  setOrder() {
    ...
    clickgoods.getOrderId({
      ...
      success: function (orderNoData) {
        ...
        api.uploadOrder(uploadData, function (res, result) {
          my.hideLoading();
          if (res) {
      ...
      fail(err) {
        console.log('clickgoods.getOrderId失败err', err)
        my.alert({
          title: '错误'
        });
      }
  ...
  sendOrderToServer(event) {
    ...
    if (this.data.giveGoodsData.length > 0) { //如果存在赠送商品
      ...
      console.log('不存在赠送商品，this.data.giveGoodsData.length <= 0')
    }
    _this.setOrder()
  }
});
